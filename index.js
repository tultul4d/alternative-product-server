...
async function run() {
  try {
    ...
    app.get('/product', async (req, res) => {
      ...
      const cursor = productCollection.find(); // Get the cursor for all products
      const result = await cursor.skip(skip).limit(parseInt(limit)).toArray(); // Apply pagination
  
      const totalItems = await productCollection.countDocuments(); // Total documents count
    ...
    //   const totalItems = await productCollection.countDocuments(); // get the total number of documents
    //   const totalPages = Math.ceil(totalItems / limit); // calculate total pages
